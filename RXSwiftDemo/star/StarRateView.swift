...
@objc protocol StarReteViewDelegate {
    //返回星星评分的分值
    @objc optional func starRate(view starRateView: StarRateView, count: Float) -> ()
}

class StarRateView: UIView {

    var delegate: StarReteViewDelegate?
    ...
    // MARK: - 对象实例化
        ...
        init(frame: CGRect,numberOfStars: Int ,currentStarCount: Float) {
            ...
            starForegroundView = starViewWithImageName("b27_icon_star_yellowg")
            addSubview(starForegroundView!)

            starBackgroundView = starViewWithImageName("b27_icon_star_gray")
